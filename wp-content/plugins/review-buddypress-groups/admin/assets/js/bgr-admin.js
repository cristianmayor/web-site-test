...
jQuery( document ).ready(
	function(){
		...
		jQuery( document ).on(
			'click', '#bgr-save-admin-criteria-settings', function(){
				jQuery( this ).addClass( 'bgr-btn-ajax' );
				jQuery( '.bgr-admin-criteria-settings-spinner' ).show();
				var field_values = [];
				var criteria     = [];
				jQuery( "input[name=BGRDynamicTextBox]" ).each(
					function () {
						if (jQuery( this ).val().trim() != '') {
							var criteria_val = jQuery( this ).val();
							field_values.push( criteria_val );
							jQuery( this ).siblings( '.bgr-switch' ).children('.bgr-criteria-state').attr( 'data-attr', criteria_val );
						}
					}
				);
				jQuery( ".bgr-criteria-state" ).each(
					function () {
				...
				jQuery.post(
					...
					function ( response ) {
						if ( response === 'admin-criteria-settings-saved' ) {
							...
							jQuery( '#bgr-save-admin-criteria-settings' ).removeClass( 'bgr-btn-ajax' );
		...
		jQuery( document ).on(
			'click', '#bgr-save-admin-general-settings', function(){
				jQuery( this ).addClass( 'bgr-btn-ajax' );
				...
				var bgr_multi_reviews        = jQuery( '#bgr-multi-reviews' ).is( ':checked' ) ? 'yes' : 'no';
				var bgr_auto_approve_reviews = jQuery( '#bgr-auto-approve-reviews' ).is( ':checked' ) ? 'yes' : 'no';
				...
				var bgr_allow_notification   = jQuery( '#bgr-notification' ).is( ':checked' ) ? 'yes' : 'no';
				...
				jQuery.post(
					...
					function ( response ) {
						if ( response === 'admin-general-settings-saved' ) {
							jQuery( '#bgr-settings-updated' ).show();
							jQuery( '#bgr-save-admin-general-settings' ).removeClass( 'bgr-btn-ajax' );
		...
		jQuery( document ).on(
			'click', '#bgr-save-admin-display-settings', function(){
				jQuery( this ).addClass( 'bgr-btn-ajax' );
				...
				jQuery.post(
					...
					function ( response ) {
						if ( response === 'admin-display-settings-saved' ) {
							jQuery( '#bgr-settings-updated' ).show();
							jQuery( '#bgr-save-admin-display-settings' ).removeClass( 'bgr-btn-ajax' );
		...
		jQuery( "#bgr-field-add" ).bind(
			"click", function () {
				var div = jQuery( "<div/>" );
				div.html( BGRGetBGRDynamicTextBox( "" ) );
				jQuery( "#bgr-textbox-container" ).append( div ).css( 'class', 'rating-review-div ui-sortable-handle' );
			}
		);
		...
		/*** FAQ(s) accordion js ***/
		jQuery( document ).on(
			'click', '.bgr-faq-accordion', function() {
		...
		jQuery( document ).on(
			'click', '.bgr-approve-review', function(){
...
function BGRGetBGRDynamicTextBox( value ) {
	return '<span>&equiv;</span><input name = "BGRDynamicTextBox" class="draggable" type="text" value = "' + value + '" />&nbsp;' +
