 ...
 * @since 1.0.7
 */
if ( !function_exists( 'reign_bp_dir_mem_send_private_message_button' ) && !is_plugin_active( 'buddyboss-platform/bp-loader.php') ) {
	...
	function reign_bp_dir_mem_send_private_message_button() {
		...
		if ( function_exists('bmpro_get_activity_post_status') ) {
			$bmpro_actmeta = array();
		
			$act_post_status = bmpro_get_activity_post_status( $cuser_id, 'bmpro_spam_status' ); //get post status of of flagged activity post to check against manual moderation.
			
			$bmpro_actmeta = get_user_meta( $cuser_id, 'bmpro_flagged_member_meta', true ); //get activity meta to find list of reporters.
			...
			if( isset( $bmpro_actmeta['reporters'] ) ){
				...
				$moderation_mode = bmpro_get_post_moderation_mode( $cuser_id, 'bmpro_member_stngs' );
				$remove = bmpro_check_related_actions( $cuser_id, 'bmpro_member_actions', 'remove_pm' );
				if( $remove ){
					if ( (is_bmpro_auto_mod( 'bmpro_member_stngs' ) && $total_flags >= $auto_mod_limit) || ( 'awaiting_moderation' == $act_post_status ) || ( 'spam' ==  $act_post_status ) ){
						return false;
					}
				}
				if ( !empty( $bmpro_actmeta ) && in_array( 'remove_pm', $bmpro_actmeta) ) {
					return false;
				}			
			}
		}
...
if ( !function_exists( 'rth_bp_member_header_meta_social_links' ) ) {
	...
	function rth_bp_member_header_meta_social_links() {
		...
		foreach ( reign_get_user_social_array() as $field_slug => $social ) {
			...
			$html_to_render	 .= '</a>';
			$html_to_render	 .= '</li>';
			if ( $counter == count( reign_get_user_social_array() ) ) {
				$html_to_render .= '</ul>';
			}
		}
		$html_to_render .= '</div>';
