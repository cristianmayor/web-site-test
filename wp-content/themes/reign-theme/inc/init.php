...
if ( !function_exists( 'reign_setup' ) ) {
	...
	function reign_setup() {
		...
		add_theme_support( 'post-formats', array(
			'aside', 'image', 'gallery', 'video', 'audio', 'quote', 'link',
		) );
...
if ( !function_exists( 'reign_scripts' ) ) {
	...
	function reign_scripts() {
		...
		if ( class_exists( 'Easy_Digital_Downloads' ) ) {
			wp_enqueue_style( 'reign_edd_style', get_template_directory_uri() . $css_path . '/edd-main.css', '', time() );
		}

		// bbPress Main CSS Enqueue
		if ( class_exists( 'bbPress' ) ) {
			wp_enqueue_style( 'reign_bbpress_style', get_template_directory_uri() . $css_path . '/bbpress-main.css', '', time() );
		}

		// PeepSo Main CSS Enqueue
		if ( class_exists( 'PeepSo' ) ) {
			wp_enqueue_style( 'reign_peepso_style', get_template_directory_uri() . $css_path . '/peepso-main.css', '', time() );
		}

		// Events Calendar Main CSS Enqueue
		if ( class_exists( 'Tribe__Events__Main' ) ) {
			wp_enqueue_style( 'reign_eventscalendar_style', get_template_directory_uri() . $css_path . '/eventscalendar-main.css', '', time() );
		}
		...
		if ( class_exists( 'BuddyPress' ) ) {
			if ( 'legacy' === $theme_package_id ) {
				wp_enqueue_style( 'reign_legacy_style', get_template_directory_uri() . $css_path . '/legacy-main.css', '', time() );
			}

			// BuddyPress Nouveau Main CSS Enqueue
			if ( 'nouveau' === $theme_package_id ) {
				wp_enqueue_style( 'reign_nouveau_style', get_template_directory_uri() . $css_path . '/nouveau-main.css', '', time() );
			}
		}
                
                // BB Platform Main CSS Enqueue
		if ( is_plugin_active( 'buddyboss-platform/bp-loader.php' ) ) {
                    wp_enqueue_style( 'reign_bb_platform', get_template_directory_uri() . $css_path . '/bb-platform-main.css', '', time() );
		}
                
		// Site AMP CSS Enqueue
		if ( function_exists( 'amp_is_request' ) && amp_is_request() || ( function_exists('amp_is_legacy') && amp_is_legacy() )) {		
			wp_enqueue_style( 'reign_amp', get_template_directory_uri() . '/assets/css/reign-amp.css', '', time() );
		}

		// Site Main CSS Enqueue
		wp_enqueue_style( 'reign_main_style', get_template_directory_uri() . $css_path . '/main.css', '', time() );

		// WooCommerce Main CSS Enqueue
		if ( class_exists( 'woocommerce' ) ) {
			wp_enqueue_style( 'reign_woocommerce_style', get_template_directory_uri() . $css_path . '/woocommerce-main.css', '', time() );
		}
		...
		if ( $font_awesome_version === 'option-1' ) {
			...
			wp_enqueue_style( 'font-awesome-both', get_template_directory_uri() . '/assets/font-awesome/css/font-awesome-both.css', '', time() );
                ...
                // Load theme font
                wp_enqueue_style( 'theme-font', get_template_directory_uri() . '/assets/fonts/fonts.css', '', time() );
...
// if ( !function_exists( 'reign_header_masthead' ) ) {
// 	/**
// 	 * Site Header
// 	 */
// 	function reign_header_masthead() {
// 		get_template_part( 'template-parts/masthead', apply_filters( 'reign_masthead_type', '' ) );
// 	}
// 	add_action( 'reign_masthead', 'reign_header_masthead' );
// }
