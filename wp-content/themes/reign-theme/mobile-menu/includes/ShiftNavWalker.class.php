...
/*
 * Set Default class for Buddypress user menu
 */

add_filter( 'nav_menu_link_attributes', 'reign_nav_menu_link_attributes', 99, 4 );
function reign_nav_menu_link_attributes( $atts, $item, $args , $depth) {
	//if ( isset($atts['icon']) && $atts['icon'] == '' && $args->menu == 'user-profile-menu' ) {
	if ( isset($atts['icon']) && $atts['icon'] == '' ) {
		$atts['icon'] = '<i class="shiftnav-icon fa fa-file-o" aria-hidden="true"></i>';
		
		if ( !empty($item->classes) && in_array( 'bp-activity-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-tasks" aria-hidden="true"></i>';
		}		
		
		if ( !empty($item->classes) && in_array( 'bp-profile-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-user" aria-hidden="true"></i>';
		}
		if ( !empty($item->classes) && in_array( 'bp-notifications-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-bell" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-messages-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-envelope" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-friends-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-users" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-groups-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-users" aria-hidden="true"></i>';
		}
		if ( !empty($item->classes) && in_array( 'bp-forums-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-wpforms" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-settings-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-cog" aria-hidden="true"></i>';
		}
		if ( !empty($item->classes) && in_array( 'bp-logout-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-sign-out" aria-hidden="true"></i>';
		}
		if ( !empty($item->classes) && in_array( 'bp-login-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-sign-in" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-register-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-user-circle-o" aria-hidden="true"></i>';
		}
		
		if ( !empty($item->classes) && in_array( 'bp-docs-nav', $item->classes)) {
			$atts['icon'] = '<i class="shiftnav-icon fa fa-file-text" aria-hidden="true"></i>';
		}
...
add_filter( 'nav_menu_css_class', 'reign_nav_menu_css_class', 99, 4);
function reign_nav_menu_css_class( $classes, $item, $args , $depth ) {
	$classes[] = 'shiftnav-has-icon';	
	return $classes;
}