...
add_action( 'login_head', 'remove_shiftnav_login_customizer');
function remove_shiftnav_login_customizer() {	
...
function _shiftnav_toggle( $target_id, $content = '', $args = array() ) {
	...
	}elseif ( $icon == 'peepso-user-menu' ) {
		$current_user = wp_get_current_user();
		if ( ($current_user instanceof WP_User ) ) {			
			$user       = PeepSoUser::get_instance( get_current_user_id() );
			?>
			<img src="<?php echo $user->get_avatar();?>" alt="<?php echo $user->get_fullname();?> avatar" title="<?php echo $user->get_profileurl();?>" class="peepso-user-avatar avatar">
			<?php
		}
	} elseif ( $icon ) {
...
function shiftnav_register_theme_locations() {
	register_nav_menu( 'shiftnav', __( 'Left Mobile Panel', 'reign' ) );
}
...
function shiftnav_register_skins() {
	...
	shiftnav_register_skin( 'reign_default', 'Default', $main . 'default.css' );
	shiftnav_register_skin( 'reign_clean', 'Clean', $main . 'clean.css' );
	shiftnav_register_skin( 'reign_dark', 'Dark', $main . 'dark.css' );
	shiftnav_register_skin( 'reign_ectoplasm', 'Ectoplasm', $main . 'ectoplasm.css' );
	shiftnav_register_skin( 'reign_sunrise', 'Sunrise', $main . 'sunrise.css' );
	shiftnav_register_skin( 'reign_coffee', 'Coffee', $main . 'coffee.css' );
}
