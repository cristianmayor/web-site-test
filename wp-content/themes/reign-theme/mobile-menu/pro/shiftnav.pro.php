...
function shiftnav_content_before( $instance_id ){
	if ( shiftnav_op( 'content_before' , $instance_id )  != '' ){ 
		echo "<div class='shiftnav-nav-before-content'>";
...
function shiftnav_content_after( $instance_id ){
	if ( shiftnav_op( 'content_after' , $instance_id )  != '' ){ 
		echo "<div class='shiftnav-nav-after-content'>";
...
function shiftnav_content_user_avatar( $instance_id ){
	
	if ( shiftnav_op( 'display_user_avatar', $instance_id ) == 'on' || shiftnav_op( 'display_user_avatar', $instance_id ) == '1' && is_user_logged_in() ):
		$current_user = wp_get_current_user();	
		echo '<div class="user-link-wrap shiftnav-menu-user-avatar-section">';
		if ( class_exists( 'PeepSo' ) ) {
			$user       = PeepSoUser::get_instance( get_current_user_id() );
			$user_link = $user->get_profileurl();
			echo '<a class="user-link" href="' . $user_link . '">';		
			echo '<img src="' . $user->get_avatar() . '" alt="' . $user->get_fullname() . ' avatar" title="' . $user->get_profileurl() . '" class="peepso-user-avatar avatar">';
			?>
...
add_action( 'shiftnav_before' , 'shiftnav_content_user_avatar' , 21 );
